...
class Train_Args:
    ...
    def gene_hmm_dic(pi_path='result_file/hmm/pi.txt', a_path='result_file/hmm/a.txt',
    ...
    def tag_txt(train_txt=Train_File):
        ...
        for line in lines:
            if line == '\n':
                continue
            line_word, line_tag = Train_Args.tag_line(line)  # 得到一行标注
        ...
        Train_Args.gene_hmm_dic()  # 将参数写入文本文件中

    @staticmethod  # 用于从文件中读取训练好的参数并按既定格式解析文本内容
    def get_args(pi_path='result_file/hmm/pi.txt', a_path='result_file/hmm/a.txt',
                 b_path='result_file/hmm/b.txt', word_path='result_file/dic/uni_gram_dic.txt'):
        Train_Args.init()
